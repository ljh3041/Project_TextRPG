...
{
	return Character::GetInstance()->GetLevel() * 3;
}
...
void VampiricDagger::UseItem()
{
	...
	cout << "데미지의 30%만큼 흡혈하는 무게 1, 공격력 10 단검의 효과: 체력" << Character::GetInstance()->GetAttack() * 3 / 10 << "회복" << endl;
}
...
int HP_Blade4::GetAttack()
{
	...
	else
	{
		...
		cout << "4번째 공격에 나의 현재체력의 5%의 추가 피해를 주는 무게2, 체력 50의 도의 효과: " << Character::GetInstance()->GetMaxHealth() * 5 / 100 << "의 추가 데미지" << endl;
	}
}
